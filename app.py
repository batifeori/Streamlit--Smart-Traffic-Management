...
def load_data():
    ...
    if not os.path.exists(file_path):
        st.error("Dataset file not found. Please upload smart_traffic_management_dataset.csv")
        st.stop()
...
def load_model():
    ...
    if not os.path.exists(model_path):
        st.error("Model file not found. Please upload traffic_volume_model.pkl")
        st.stop()
...
if page == "Dashboard Overview":
    ...
    st.markdown("Real-time traffic insights and monitoring dashboard.")
    ...
    st.subheader("Traffic vs Vehicle Speed")
...
# -------------------------------------------------------
    ...
    accident_reported = col3.selectbox(
        "Accident Reported",
        [0,1]
    )
